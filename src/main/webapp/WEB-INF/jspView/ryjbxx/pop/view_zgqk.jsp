...
<table id="zgqk_table" data-keyid="${zgqk.NId}" data-fydm="${zgqk.NFy}"
	data-rybh="${zgqk.NRybh}">
	<tr style="height:35px">
		<td class="odd_td" style="font-weight:bold;">批准文号：</td>
		<td class="even_td"><input type="text" class="myLabel" value="${zgqk.CPzwh}" /></td>
		<td class="odd_td" style="font-weight:bold;">变动时间：</td>
		<td class="even_td"><input type="text" id="checkDate1" class="myDate_label" value="${zgqk.DBgsj}" /></td>
	</tr>
	<tr style="height:35px">
		<td class="odd_td">变动前部门：</td>
		<td class="even_td"><div class="mySelect">
				<select class="my_select_s plxgUp" name="CBmbefore">
					<c:forEach items="${listDmBm}" var="listDmBm">
						<option value="${listDmBm.NDm}" ${listDmBm.CMc==zgqk.CBmbefore?'selected="selected"':"" }>${listDmBm.CMc}</option>
					</c:forEach>
				</select><input type="text" class="my_input_s"  value="${zgqk.CBmbefore}"/></div></td>
		<td class="odd_td">变动后部门：</td>
		<td class="even_td"><div class="mySelect">
				<select class="my_select_s plxgUp" name="CBmafter">
					<c:forEach items="${listDmBm}" var="listDmBm">
						<option value="${listDmBm.NDm}" ${listDmBm.CMc==zgqk.CBmafter?'selected="selected"':"" }>${listDmBm.CMc}</option>
					</c:forEach>
				</select><input type="text" class="my_input_s"  value="${zgqk.CBmafter}"/></div></td>
	</tr>
	<tr>
